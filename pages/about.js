    ...
    >
      ...
      <div className="w-6/12 flex justify-between overflow-hidden">
        <div className="invisible hidden md:inline-block" ref={addRef}>
          <Link href="/" className="headerLink">Overview</Link>
        </div>
        <div className="invisible" ref={addRef}>
...
export default function About() {
  ...
  const footerLeftRefs = useRef([]);
  const footerRightRefs = useRef([]);

  const collect = (store) => (el) => {
    if (el && !store.current.includes(el)) store.current.push(el);
  };
  const addContentRef = collect(contentRefs);
  const addFooterLeftRef = collect(footerLeftRefs);
  const addFooterRightRef = collect(footerRightRefs);
            ...
            >
              <div className="flex flex-col">
                ...
                <div className="w-full md:w-10/12 font-light text-base">
                  ...
                  invito a scriverti. Sostituisci questo testo in pages/about.js.
                </div>
              </div>
